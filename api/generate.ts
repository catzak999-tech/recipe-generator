...
export default async function handler(req: Request) {
  ...
  // Only block if Origin is present and not allowlisted
  if (origin && !okCors(origin)) {
  ...
  // Pull args, choose a JSON-mode-capable default model
  const {
    messages,
    model,
    temperature = 0.7,
  ...
  // Force a model that supports response_format JSON mode
  ...
  const payload = {
    ...
    temperature,
    max_tokens,
    // This forces OpenAI to return a single JSON object with no extra text
  ...
  // Pass through as text; client parses JSON
